...
for actual_dotfile in $HOME/.dotfiles/.*
    do
        dotfile=$(echo $actual_dotfile | awk -F"$dotfiles_loc/" '{print 
$2}')
    ...
    processDotDir() {
        ...
        if [ "$remove" = "cleanup" ]; then
            if [ -d "$dot_directory" ]; then
                echo -n "Are you sure you want to delete $dot_directory? 
[n]: "
                read REMOVE_DIR
        ...
        elif [[ ! -d "$dot_directory" ]] && [[ ! -a "$dot_directory" ]]; 
then
            mkdir -p "$dot_directory" && echo "created a $dot_directory 
directory"
...
then
    ...
    if [[ -d "$BUILDOUT_DIR" ]] && [ ! -e "$BUILDOUT_DIR/default.cfg" ]; 
then
...
EOF
        ...
        echo
        echo "A default buildout config has been set up for you in 
$BUILDOUT_DIR"
        echo "You can modify the $BUILDOUT_DIR/default.cfg to your 
liking"
        echo
    fi
